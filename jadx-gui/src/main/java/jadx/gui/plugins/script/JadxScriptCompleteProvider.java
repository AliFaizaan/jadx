...
public class JadxScriptCompleteProvider extends CompletionProviderBase {
	...
	private static Map<String, Icon> buildIconsMap() {
		...
		map.put("class", UiUtils.ICON_CLASS);
		map.put("method", UiUtils.ICON_METHOD);
		map.put("field", UiUtils.ICON_FIELD);
		map.put("property", UiUtils.ICON_PROPERTY);
		map.put("parameter", UiUtils.ICON_PARAMETER);
		map.put("package", UiUtils.ICON_PACKAGE);
		return map;
	}
	...
	private List<Completion> convertCompletions(List<SourceCodeCompletionVariant> completions, String code, int replacePos) {
		...
		for (int i = 0; i < count; i++) {
			...
			cmpl.setIcon(icon != null ? icon : UiUtils.FILE_ICON);
			list.add(cmpl);
		}
		return list;
	}
